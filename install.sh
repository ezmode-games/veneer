...
detect_platform() {
  ...
  case "${os}" in
    Linux)  echo "linux" ;;
    Darwin) echo "macos" ;;
    *)      error "Unsupported operating system: ${os}. Veneer supports Linux and macOS." ;;
  esac
}
...
resolve_version() {
  ...
  local version="${1:-${VENEER_VERSION:-}}"
  if [ -n "${version}" ]; then
...
main() {
  ...
  if [ "${platform}" = "linux" ] && [ "${arch}" = "arm64" ]; then
    error "linux-arm64 builds are not available yet. See https://github.com/${REPO}/issues for updates."
  fi

  info "Detected platform: ${platform}-${arch}"

  version="$(resolve_version "${version}")"
  info "Installing Veneer ${version}..."
  ...
  if [ -n "${installed_version}" ]; then
    ...
    success "Installed Veneer ${version} to ${INSTALL_DIR}/${BINARY_NAME}"
  fi
}
